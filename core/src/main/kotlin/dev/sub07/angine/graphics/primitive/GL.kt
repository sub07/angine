...
class GLTexture(
    ...
    override fun sendData(data: ByteBuffer) {
        glTextureSubImage2D(
            ...
            width,
            height,
            externalStorageFromNbChannel(format),
            GL_UNSIGNED_BYTE,
            data
        )
    }
...
class ResizableGLFramebuffer(initial: Framebuffer, val create: (Int, Int) -> Framebuffer) :
    ResizableObject<Framebuffer>, Framebuffer, Bindable {
...
class GLGraphicObjectFactory : GraphicObjectFactory, KoinComponent {
    ...
    override fun makeAutoResizedFramebuffer(width: Int, height: Int): Framebuffer {
        ...
        return ResizableGLFramebuffer(makeFramebuffer(width, height)) { w, h ->
            makeFramebuffer(w, h)
        }.also { angine.resizeEventListeners += it }
    }
